...
INSTANCE_ID="i-044fe5ea4f0bda534" #Replace 
...
CPU_UTILIZATION=$(aws cloudwatch get-metric-statistics --namespace AWS/EC2 --metric-name CPUUtilization --period 300 \
 --statistics Maximum --dimensions Name=InstanceId,Value=$INSTANCE_ID \
 --start-time $(date -u +"%Y-%m-%dT%H:%M:%SZ" -d '10 minutes ago') --end-time $(date -u +"%Y-%m-%dT%H:%M:%SZ")\
...
# else
#     echo "CPU utilization is normal: $CPU_UTILIZATION%."
fi
